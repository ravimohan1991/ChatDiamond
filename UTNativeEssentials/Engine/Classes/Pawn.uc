...
replication
{
	...
	// OldUnreal: as of v469c, ViewRotation also replicates if bNetViewingOwner==true		 
	unreliable if( (bNetOwner && bIsPlayer && bNetInitial && Role==ROLE_Authority) || bDemoRecording )
		ViewRotation;
	// OldUnreal: as of v469c, MoveTarget also replicates if bNetViewingOwner==true		
...
static function bool SetSkinElement(Actor SkinActor, int SkinNo, string SkinName, string DefaultSkinName)
{
		...
		&& (DefaultSkinName != "") )
	{
		...
		pkg = Left(SkinName, InStr(SkinName, "."));
...
simulated function bool AdjustHitLocation(out vector HitLocation, vector TraceDir)
{
	...
	if ( HasAnim(AnimSequence) && (GetAnimGroup(AnimSequence) == 'Ducking') && (AnimFrame > -0.03) )
	{
