...
var() globalconfig bool bNoMouseSmoothing;
...
// UTPG --
var int 	LoginAttempts;	// How many login attempts we've made
var bool	bLoginDisabled;	// no longer able to login even if correct password
var float	NextLoginTime;	// Time at which game will accept another attempt to login

// All received from GameInfo
var float 	ViewDelay;		// Seconds between each viewplayer()/viewplayernum()
var float 	TauntDelay;		// Seconds between taunts
var float 	SpeechDelay;	// Seconds between chat messages

var float 	LastView;		// Used to track last view
var float 	LastTaunt;		// Used to track last taunt
var float 	LastSpeech;		// Used to track last chat message

var bool	bCyclingView;	// Used to allow ViewClass to disregard LastView

var float 	MinFOV;			// Enforced MinFOV for this server
var float 	MaxFOV;			// Enforced MaxFOV for this server
...
replication
{
	...
	// OldUnreal: as of v469c, all of the variables below also replicate if bNetViewingOwner==true	
...
exec function ViewPlayerNum(optional int num)
{
	...
	{
		if (ViewDelay > 0.0)
			if (bCyclingView || ((Level.TimeSeconds - LastView) < ViewDelay))
				return;
...
{
	if ( (Level.TimeSeconds < NextLoginTime) || bLoginDisabled )
		  return;
	...
	if (LoginAttempts >= Level.Game.MaxLoginAttempts)
	{
		...
		switch (Level.Game.ActionToTake)
		{
			case DO_KickBanPlayer:
				KickBanMe("Too many login attempts."); break;

			case DO_KickPlayer:
				KickMe("Too many login attempts."); break;

			case DO_DisableLogin:
				bLoginDisabled = True; break;

			case DO_Log:
				log(LoginAttempts@"incorrect admin login attempts from"@PlayerReplicationInfo.PlayerName@GetPlayerNetworkAddress());
		}
	}
}
...
)
{
	...
	if ( PendingMove != None )
	{
		...
		if ( PendingMove.CanMergeAccel(NewAccel) &&
   		     // stijn: force pending move to be flushed if we press jump
			 // That way, the jump moves becomes its own unmerged move
		     bPressedJump == PendingMove.bPressedJump ) 
		{
...
//			Log("Bursting move"@Level.TimeSeconds);
	...
	if ( PendingMove.CanBuffer(NewAccel) &&
...
singular function ServerTaunt(name Sequence )
{
	if (Level.TimeSeconds - LastTaunt > TauntDelay && GetAnimGroup(Sequence) == 'Gesture')
	{
//		if (IsAnimating())
//			FinishAnim();
		PlayAnim(Sequence, 0.7, 0.2);
...
exec function Speech( int Type, int Index, int Callsign )
{
	...
	if (Level.TimeSeconds - OldMessageTime > SpeechDelay)
	{
...
//	if (Level.TimeSeconds - LastSpeech < SpeechDelay)
//		return;
...
final function KickMe(optional string Reason)
{
	if (Level.Game.bLogAdminActions)
		log(PlayerReplicationInfo.PlayerName@"("$GetPlayerNetworkAddress()$") was kicked from the server:"@Reason,'AdminAction');
	Destroy();
}

final function KickBanMe( optional string Reason)
{
	local string IP;
	local int j;

	IP = GetPlayerNetworkAddress();
	if ( Level.Game.CheckIPPolicy(IP) )
	{
		IP = class'InternetInfo'.static.StripPort(IP);
		if ( Level.Game.bLogAdminActions )
			log(PlayerReplicationInfo.PlayerName@"("$GetPlayerNetworkAddress()$") was banned from the server:"@Reason,'AdminAction');
...
exec function KickList()
{
	...
	for (P = Level.PawnList; P != None; P = P.NextPawn)
		if (P.bIsPlayer && (PlayerPawn(P) == None || NetConnection(PlayerPawn(P).Player) != None))
			ClientMessage(Right("   "$P.PlayerReplicationInfo.PlayerId, 3) $ ")" @ P.PlayerReplicationInfo.PlayerName);
}
...
exec function KickId(string S)
{
	...
	{
		if (P == Self) continue;
		if (P.bIsPlayer && P.PlayerReplicationInfo != None && (PlayerPawn(P)==None || NetConnection(PlayerPawn(P).Player)!=None) &&
			string(P.PlayerReplicationInfo.PlayerId) == S)
		{
			if (Level.Game.bLogAdminActions)
				log(PlayerReplicationInfo.PlayerName@"kicked"@P.PlayerReplicationInfo.PlayerName@PlayerPawn(P).GetPlayerNetworkAddress()@"from"@Left(string(Level), InStr(string(Level), "."))$".unr",'AdminAction');
			ClientMessage(PlayerReplicationInfo.PlayerName@"kicked"@P.PlayerReplicationInfo.PlayerName@PlayerPawn(P).GetPlayerNetworkAddress()@"from"@Left(string(Level), InStr(string(Level), "."))$".unr");
			P.Destroy();
			return;
		}
	}
			...
			&&	PlayerPawn(aPawn)!=None && NetConnection(PlayerPawn(aPawn).Player) != None )
		{
			...
			if ( Level.Game.CheckIPPolicy(IP) )
			{
				...
				if ( j < ArrayCount(Level.Game.IPPolicies) )
					Level.Game.IPPolicies[j] = "DENY,"$IP;
				if ( Level.Game.bLogAdminActions )
					log(PlayerReplicationInfo.PlayerName@"banned"@aPawn.PlayerReplicationInfo.PlayerName@PlayerPawn(aPawn).GetPlayerNetworkAddress()@"during"@Left(string(Level), InStr(string(Level), "."))$".unr",'AdminAction');
				Level.Game.SaveConfig();
			}
			else if ( Level.Game.bLogAdminActions )
				log(PlayerReplicationInfo.PlayerName@"kicked"@aPawn.PlayerReplicationInfo.PlayerName@PlayerPawn(aPawn).GetPlayerNetworkAddress()@"from"@Left(string(Level), InStr(string(Level), "."))$".unr",'AdminAction');
			...
			string(P.PlayerReplicationInfo.PlayerId) == S)
		{
			...
			if(Level.Game.CheckIPPolicy(IP))
			{
				...
				if ( j < ArrayCount(Level.Game.IPPolicies) )
					Level.Game.IPPolicies[j] = "DENY,"$IP;
				if ( Level.Game.bLogAdminActions )
					log(PlayerReplicationInfo.PlayerName@"banned"@P.PlayerReplicationInfo.PlayerName@PlayerPawn(P).GetPlayerNetworkAddress()@"during"@Left(string(Level), InStr(string(Level), "."))$".unr",'AdminAction');
				Level.Game.SaveConfig();
			}
			else if ( Level.Game.bLogAdminActions )
				log(PlayerReplicationInfo.PlayerName@"kicked"@P.PlayerReplicationInfo.PlayerName@PlayerPawn(P).GetPlayerNetworkAddress()@"from"@Left(string(Level), InStr(string(Level), "."))$".unr",'AdminAction');
...
exec function SetMaxMouseSmoothing( int Mode )
{
	if ( Mode < 0 || Mode > 2 )
		Mode = 1;
	switch ( Mode )
	{
		case 0:
			...
			break;
		case 1: //Normal pre-v469 behaviour
			bMaxMouseSmoothing = false;
...
exec function SwitchWeapon (byte F )
{
	...
	if ( bShowMenu || Level.Pauser!="" || Level.Game.bGameEnded )
	{
//		if ( myHud != None )
//			myHud.InputNumber(F);
		return;
...
exec function ViewClass( class<actor> aClass, optional bool bQuiet )
{
	...
	if (ViewDelay > 0.0)
		if (((Level.TimeSeconds - LastView) < ViewDelay) && !bCyclingView)
			return;
	if (!bCyclingView)
		LastView = Level.TimeSeconds;
...
function StartWalk()
{
    ...
    if ( NetConnection(Player) != None && RemoteRole == ROLE_AutonomousProxy )
    {
        OldRole = RemoteRole;
		...
		if ( PTarget != None )
		{
			if ( Level.NetMode == NM_Client )
			{
				...
				if ( PTarget.Weapon != None )
				{
					...
					// OldUnreal: fix for OldUnreal/UnrealTournamentPatches/issues/767
					if (Abs(PTarget.Weapon.PlayerViewOffset.X - PTarget.Weapon.Default.PlayerViewOffset.X) <= 1.0 && 
						Abs(PTarget.Weapon.PlayerViewOffset.Z - PTarget.Weapon.Default.PlayerViewOffset.Z) <= 1.0)
						PTarget.Weapon.PlayerViewOffset *= 100; //scale since network passes vector components as ints
				}
			}
			if ( PTarget.bIsPlayer )
...
ignores SeePlayer, HearNoise, Bump;
	...
	function ServerFeignDeath()
	{
		...
		if ( Weapon != None ) {
			Weapon.PutDown();
			if ( Level.Game.bFixMultiWeaponBug && !W.IsInState('DownWeapon') )
				W.GotoState('DownWeapon');
		}
		PendingWeapon = W;
		GotoState('FeigningDeath');
	}
...
ignores SeePlayer, HearNoise, Bump;
	...
	{
		Global.ServerMove_v2(TimeStamp, InAccel, ClientLoc, MoveFlags, MergeCount, DodgeMove, ClientRoll, (32767 & (Rotation.Pitch/2)) * 32768 + (32767 & (Rotation.Yaw/2)));
	}
	...
	function BeginState()
	{
		...
		if ( Level.Game != None && Level.Game.bFixFeignDeathZoomBug )
		    EndZoom();
		if ( carriedDecoration != None )
			DropDecoration();
		NewRot = Rotation;
