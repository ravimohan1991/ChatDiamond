...
// UTPG
var globalconfig float 	PlayerViewDelay;
var globalconfig float 	PlayerSpeechDelay;
var globalconfig float 	PlayerTauntDelay;
...
var config bool bFixMultiWeaponBug;
var config bool bFixFeignDeathZoomBug;
...
//
var globalconfig bool	bLogAdminActions;
var globalconfig float	LoginDelaySeconds;	// Seconds after unsuccessful login before allowing another attempt
var globalconfig int	MaxLoginAttempts;	// Maximum number of unsuccessful logins from a player before taking action
var globalconfig enum ELoginAction
{	DO_Nothing,			// Do nothing (standard)
	DO_Log,				// Write alert to server log
	DO_DisableLogin,	// Disable this player's ability to login as admin for the remainder of the current game
...
function AdminLogin( PlayerPawn P, string Password )
{
	...
	if (Password == AdminPassword)
	{
		...
		if (bLogAdminActions)
			log(P.PlayerReplicationInfo.PlayerName@"logged in as administrator from"@P.GetPlayerNetworkAddress(),'Admin');
		else Log("Administrator logged in.");
...
function AdminLogout( PlayerPawn P )
{
	...
	if (P.bAdmin)
	{
		...
		if ( P.ReducedDamageType == 'All' )
			P.ReducedDamageType = '';
		P.StartWalk();
		OldScore = P.PlayerReplicationInfo.Score;
		P.Suicide();
		P.PlayerReplicationInfo.Score = OldScore;
		if (bLogAdminActions)
			log(P.PlayerReplicationInfo.PlayerName@"logged out as administrator.");
		else Log("Administrator logged out.");
...
function PreBeginPlay()
{
	...
	// Make sure MaxFOV cannot be set below MinFOV,
	// Also make sure we have valid values
	MinFOV = FClamp(MinFOV, 1.0, 130.0);
	MaxFOV = FClamp(MaxFOV, MinFOV, 170.0);
...
function string GetInfo()
{
	...
	if ( WorldLog != None && !WorldLog.bWorldBatcherError )
...
event PostLogin( playerpawn NewPlayer )
{
	...
	// Set all expoit limits
	NewPlayer.ViewDelay = PlayerViewDelay;
	NewPlayer.SpeechDelay = PlayerSpeechDelay;
	NewPlayer.TauntDelay = PlayerTauntDelay;
	NewPlayer.MinFOV = Max( MinFOV, 1 );
	NewPlayer.MaxFOV = Min( MaxFOV, 170 );
	NewPlayer.MaxNameChanges = MaxNameChanges;
}
...
{
	M.NextMessageMutator = MessageMutator;
	MessageMutator = M;
}
...
function bool AllowsBroadcast( actor broadcaster, int Len )
{
	...
	P = PlayerPawn(BroadCaster);
	if ( P != None )
	{
		// prevent message spamming
		if ( (Level.TimeSeconds - P.LastMessageWindow < 20)
			&& (Level.TimeSeconds > 20) )
			return false;
		P.LastMessageWindow = 0.75 * P.LastMessageWindow + 0.25 * Level.TimeSeconds;
	}
	SentText += Len;
...
function EndGame( string Reason )
{
	...
	if ( !BaseMutator.HandleEndGame() && !SetEndCams(Reason) )
	{
		bOverTime = true;
		return;
	}
...
defaultproperties
{
	...
	PlayerViewDelay=1.0
	PlayerSpeechDelay=0.3
	PlayerTauntDelay=2.0
	MinFOV=80
	MaxFOV=130
//	NUMPOLICIES=256
	NoLockdown=1
