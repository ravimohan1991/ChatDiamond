...
With that being written, feel free to send pull requests to \href{https://github.com/ravimohan1991/ChatDiamond/}{Chat Diamond} repository, even for earlier game versions.  For known issue(s) you may want to go to this \href{https://github.com/ravimohan1991/ChatDiamond/issues}{page}.
...
Based upon the above motivation we have a choice to make between replacing the client HUD or UT console, in order to gather the UT messages client side.  My first instinct was to replace the HUD because that would give more precise classification and control of the displayable messages, in the sense Epic wrote the code, like \href{http://uncodex.ut-files.com/UT/v436/Source_botpack/challengehud.html}{so} (line 1499)
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{consoleundhud}
...
\item Spectators
\end{itemize}
and both of them are \ClassColor{Pawn} objects. Table \ref{tab:messageform} present in APPENDIX \ref{append:messagingtables} shows the format of the messages.  The spectators' messages don't belong to `Say' or `TeamSay' categories and belong to `Event' (as MsgType).  For those `Event' types we just look for the `:' deliminator in the `Msg' to put the `Msg' in `Chat' category.  We further note that looking for single `:' deliminator may not be enough since the spectator's name may also contain `:' thus disrupting the original message.  A better way would be to first extract spectator name by repeatedly checking with the spectator list (PRIArray) and proceeding forward on finding appropriate match.  We leave that for the future development. 
...
I have been using combination of Wot Greal (unrealscript code) and \href{https://www.qt.io/product/development-tools}{QT Creator} (Linux C++) along with Microsoft Visual Studio 2022 (Windows C++) IDEs for developing Chat Diamond.  
...
\begin{itemize}
\item \texttt{guard} is defined like \href{https://github.com/ravimohan1991/ChatDiamond/blob/4d218e50b6ac9a57d7404ede8f4e333e080d8600/UTNativeEssentials/Core/Inc/UnFile.h#L211}{so}
\lstset{language=C++}
\begin{lstlisting}[frame=single]
#define guard(func)			
{
	...
	{
\end{lstlisting}
Please note that we have schemetically displayed (written) the code for clarity.  The actual ``\#define'' code would either require newline `\textbackslash' or would be a single line definition.        
\item \texttt{unguard} is defined like \href{https://github.com/ravimohan1991/ChatDiamond/blob/4d218e50b6ac9a57d7404ede8f4e333e080d8600/UTNativeEssentials/Core/Inc/UnFile.h#L212}{so}
\lstset{language=C++}
\begin{lstlisting}[frame=single]
	}
	catch(TCHAR* Err)
...
%}
%\end{lstlisting}
As evident, there are two catch clauses.  These clauses are what were supplied with the community patch SDK.  However, they require \texttt{DO\_GUARD} enabling to take effect.  Something we can consider for future releases.  For the moment, we are doing with manually written try-catch blocks.  For instance like \href{https://github.com/ravimohan1991/ChatDiamond/blob/36f05b148c3e28ebc69301ac9ef2696b11178de4/UTNativeEssentials/ChatDiamond/Src/ChatDiamondNative.cpp#L237-L244}{so}
\lstset{language=C++}
\begin{lstlisting}[frame=single]
...
\item \texttt{P\_FINISH} is defined like \href{https://github.com/ravimohan1991/ChatDiamond/blob/5592fef2b13305e441c1dd2b09dde7dd52ff2d83/UTNativeEssentials/Core/Inc/UnScript.h#L75}{so}
...
\item \texttt{IMPLEMENT\_FUNCTION} is defined like \href{https://github.com/ravimohan1991/ChatDiamond/blob/36f05b148c3e28ebc69301ac9ef2696b11178de4/UTNativeEssentials/Core/Inc/UnScript.h#L23-L37}{so}
...
#endif
\end{lstlisting}
I don't know the implementation of \texttt{GRegisterNative}, which, from the name, suggests some internal registration mechanism for native function into a \href{https://github.com/ravimohan1991/ChatDiamond/blob/36f05b148c3e28ebc69301ac9ef2696b11178de4/UTNativeEssentials/Core/Inc/UnScript.h#L13-L18}{table}. 
